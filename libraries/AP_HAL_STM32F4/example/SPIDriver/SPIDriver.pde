...
void setup (void) {
    ...
    hal.console->printf_P(PSTR("Starting AP_HAL_STM32F4::SPIDriver test\r\n"));

    spidev = hal.spi->device(AP_HAL::SPIDevice_MPU6500); // Not really MPU6500, just a generic SPU driver
    if (!spidev)
       hal.scheduler->panic(PSTR("Starting AP_HAL_STM32F4::SPIDriver failed to get spidev\r\n"));
}

void loop (void) { 
//    uint8_t tx_data[] = { 'h', 'e', 'l', 'l', 'o', 0 };
//    uint8_t rx_data[] = {  0,   0,   0,   0,   0,  0 };
    uint8_t tx_data[] = {0xf5, 0xff};
    uint8_t rx_data[] = {  0,   0};
